...
function gen_events(r::GenRecommender)
    events = [Event(r.user_dict[user], r.media_dict[media], rating) for (
        user,
        media,
        rating,
    ) in zip(r.raw_user_ids, r.raw_media_ids, r.ratings)]
    return events
end

gen_data_accessor(e::Array, r::GenRecommender) =
    DataAccessor(e, length(r.user_dict), length(r.media_dict))
...
function inference(r::GenRecommender, m::MF, user_id, n_recs::Int)
    recs = recommend(
        m,
        haskey(r.user_dict, user_id) ? r.user_dict[user_id] : user_id,
        n_recs,
        collect(1:length(r.user_dict)),
    )
    recs = [(
        [k for (k, v) in filter(p -> last(p) == key, r.media_dict)][1],
        value,
...
function initialize(
    ...
    trained_recommender = train_recommender(
        _recommender,
        reg = reg,
        learning_rate = learning_rate,
        eps = eps,
